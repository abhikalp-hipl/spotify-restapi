...
def remove_album(album_id, access_token):
    """
    Remove an album for the current user using its ID.
    """
    ...
    if response.status_code == 200:
        print(f"Album {album_id} removed successfully!")
        return True
    else:
        print("Error removing album:", response.json())
        return False
...
if __name__ == "__main__":
    ...
    album_name = input("Enter the name of the album you want to remove: ")
    
    album_id = search_album(album_name, ACCESS_TOKEN)
    
    if album_id:
        remove_album(album_id, ACCESS_TOKEN)